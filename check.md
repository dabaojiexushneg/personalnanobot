...
> 我把访问大模型的 token 成本纳入了可观测体系，不是只在日志里看一眼。每次模型调用都会生成 Trace，Trace 里保存输入 token、输出 token 和总 token；控制台展示项目累计总 Token 消耗。演示环境里会保留一个历史基础值，让页面初始就能体现项目规模；后续真实访问大模型产生的 token 会继续基于 Trace 累加，真实生产环境则以 Trace 里的 usage 为准。
